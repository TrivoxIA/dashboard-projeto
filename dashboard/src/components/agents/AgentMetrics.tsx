...
export default function AgentMetrics({ stats, loading }: Props) {
  const items = [
    { icon: MessageSquare, label: 'Total',          value: stats.total,                               color: 'text-[var(--text-secondary)] bg-[var(--border-zinc)]/40' },
        ...
        <div key={label} className="bg-[var(--bg-page)]/40 border border-[var(--border-default)] rounded-xl p-3">
          {loading ? (
            <div className="space-y-2">
              <div className="h-3 w-12 bg-[var(--border-zinc)]/40 rounded animate-pulse" />
              <div className="h-5 w-8 bg-[var(--bg-page)]/50 rounded animate-pulse" />
            </div>
          ) : (
            <>
