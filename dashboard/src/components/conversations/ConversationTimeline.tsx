...
export default function ConversationTimeline({ startedAt, endedAt, agentName, status, notes }: Props) {
  ...
  const ICON_CFG = {
    ...
    message: { Icon: MessageSquare, cls: 'bg-slate-500/20 text-[var(--text-secondary)] border-slate-500/30' },
  ...
  return (
    <div className="space-y-1">
      <p className="text-xs font-semibold text-[var(--text-tertiary)] uppercase tracking-wider mb-3">Timeline</p>
      <div className="relative">
        {/* Linha vertical */}
        <div className="absolute left-[15px] top-0 bottom-0 w-px bg-[var(--border-zinc)]/40" />
        <div className="space-y-4">
          {events.map((ev, i) => {
            const { Icon, cls } = ICON_CFG[ev.type]
            return (
              <div key={i} className="flex items-start gap-3 relative">
                <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full border ${cls} z-10 bg-[var(--bg-card)]`}>
                  <Icon className="h-3.5 w-3.5" />
                </div>
                <div className="flex-1 pt-1">
                  <p className="text-sm text-[var(--text-primary)]">{ev.label}</p>
                  <div className="flex items-center gap-2 mt-0.5">
                    {ev.author && <span className="text-xs text-[var(--text-tertiary)]">{ev.author}</span>}
