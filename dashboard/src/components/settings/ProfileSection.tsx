      ...
      <div className="bg-[var(--bg-card)] border border-[var(--border-default)] rounded-xl p-5">
        <div className="flex items-center gap-4 mb-5">
          <div className="h-14 w-14 rounded-2xl bg-cyan-500/20 border border-cyan-500/30 flex items-center justify-center">
            <span className="text-xl font-bold text-[var(--sidebar-active-text)] uppercase">
              {user?.email?.[0] ?? 'U'}
            </span>
          </div>
          <div>
            <p className="font-semibold text-[var(--text-primary)]">{user?.email}</p>
            <span className="text-xs text-[var(--sidebar-active-text)] bg-cyan-500/15 px-2 py-0.5 rounded-full font-medium">
              Admin
            </span>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            <p className="text-xs text-[var(--text-tertiary)] mb-1">ID do Usuário</p>
            <div className="bg-[var(--bg-page)]/40 border border-[var(--border-default)] rounded-lg px-3 py-2">
              <p className="text-sm text-[var(--text-secondary)] font-mono truncate">{user?.id}</p>
            </div>
          </div>
          <div>
            <p className="text-xs text-[var(--text-tertiary)] mb-1">Criado em</p>
            <div className="bg-[var(--bg-page)]/40 border border-[var(--border-default)] rounded-lg px-3 py-2">
          ...
          <div>
            <p className="text-xs text-[var(--text-tertiary)] mb-1">Último acesso</p>
            <div className="bg-[var(--bg-page)]/40 border border-[var(--border-default)] rounded-lg px-3 py-2">
              <p className="text-sm text-[var(--text-secondary)]">
      ...
      <div className="bg-[var(--bg-card)] border border-[var(--border-default)] rounded-xl p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          {!showPwForm && (
            <button
              ...
              className="text-xs text-[var(--sidebar-active-text)] hover:text-cyan-300 bg-cyan-500/10 hover:bg-cyan-500/20 px-3 py-1.5 rounded-lg transition-colors"
            >
              Alterar
            </button>
          )}
        ...
        {showPwForm && (
          <form onSubmit={handleChangePassword} className="space-y-3">
            <div>
              <label className="block text-xs text-[var(--text-tertiary)] mb-1">Nova senha</label>
              <div className="relative">
                <input
                  ...
                  className="w-full bg-[var(--bg-page)]/50 border border-[var(--border-medium)] rounded-lg px-3 py-2 text-sm text-[var(--text-primary)] placeholder-[var(--text-placeholder)] focus:outline-none focus:border-emerald-500/50 pr-10"
                />
                <button
                  type="button"
            ...
            <div>
              <label className="block text-xs text-[var(--text-tertiary)] mb-1">Confirmar nova senha</label>
              <input
                type={showPw ? 'text' : 'password'}
                value={confirmPw}
                onChange={e => setConfirmPw(e.target.value)}
                placeholder="Repita a nova senha"
                className="w-full bg-[var(--bg-page)]/50 border border-[var(--border-medium)] rounded-lg px-3 py-2 text-sm text-[var(--text-primary)] placeholder-[var(--text-placeholder)] focus:outline-none focus:border-emerald-500/50"
              />
            </div>
            <div className="flex items-center gap-2 pt-1">
              ...
              <button
                ...
                className="flex-1 bg-[var(--bg-page)]/50 hover:bg-[var(--border-zinc)]/40 text-[var(--text-secondary)] text-sm font-medium rounded-lg py-2 transition-colors"
              >
                Cancelar
              </button>
            </div>
        ...
        {!showPwForm && !msg && (
          <p className="text-xs text-[var(--text-tertiary)]">Sua senha atual está ativa. Clique em "Alterar" para definir uma nova.</p>
        )}
      </div>
    </div>
  )
